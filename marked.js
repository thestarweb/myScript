...
if((cap=this.rules.link.exec(src))&&(cap[0][0]=='!'||cap[0][0]=='$'||cap[0][0]=='[')){src=src.substring(cap[0].length);out+=this.outputLink(cap,{href:cap[2],title:cap[3]});continue;}
...
+this.output(cap[1])
+'</del>';continue;}
if(cap=this.rules.color.exec(src)){src=src.substring(cap[0].length);
...
+' target="_blank">'
+this.output(cap[1])
+'</a>';}else if(cap[0][0]=="!"){return'<img src="'
+escape(link.href)
+'" alt="'
+escape(cap[1])
+'"'
...
+';">'
+escape(cap[1])
+'</span>';}};InlineLexer.prototype.mangle=function(text){var out='',l=text.length,i=0,ch;for(;i<l;i++){ch=text.charCodeAt(i);if(Math.random()>0.5){ch='x'+ch.toString(16);}
out+='&#'+ch+';';}
